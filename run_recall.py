...
def load_data_for_evaluation(dataset_name: str, base_data_dir: str, val_ratio: float, test_ratio: float):
    """
    【全新修订版】
    为评估加载数据，确保在任何设置下都使用完整的训练和验证历史，
    同时提供 transductive 和 inductive 两个独立的测试集。
    """
    ...
    test_list_transductive = test_df_transductive[['u', 'r', 'i', 'ts', 'label', 'idx']].values.tolist()
    test_list_inductive = test_df_inductive[['u', 'r', 'i', 'ts', 'label', 'idx']].values.tolist()

    print("--- Data Loading (Corrected Logic) ---")
    print(f"Full history for model (Train + Val): {len(full_train_list) + len(full_val_list)} interactions")
    print(f"Test Set (Transductive): {len(test_list_transductive)} interactions")
    print(f"Test Set (Inductive): {len(test_list_inductive)} interactions with new nodes")

    return entities, relations, full_train_list, full_val_list, test_list_transductive, test_list_inductive, node_num
